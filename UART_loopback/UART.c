...
void UART0_Receive_DMA() {
		...
		while (!(UART0->S1 & UART0_S1_RDRF_MASK))
			;
		receivedbuffer[pinky_index] = UART0 -> D;
		
		while (!(UART0->S1 & UART0_S1_RDRF_MASK))
			;
		receivedbuffer[ring_index] = UART0 -> D;
		
		while (!(UART0->S1 & UART0_S1_RDRF_MASK))
			;
		receivedbuffer[middle_index] = UART0 -> D;
		
		while (!(UART0->S1 & UART0_S1_RDRF_MASK))
			;
		receivedbuffer[index_index] = UART0 -> D;
		
		while (!(UART0->S1 & UART0_S1_RDRF_MASK))
			;
		receivedbuffer[thumb_index] = UART0 -> D;
