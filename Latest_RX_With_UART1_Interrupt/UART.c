...
volatile int received_go_signal = 0; 
volatile int raise_this_flag = 0;
...
void Init_UART1(void)
{
	...
	PORTC->PCR[3] |= PORT_PCR_MUX(3); //3 is the RX pin for UART1; alternative 3 is the selection option for UART1 rx
	
	sbr = (uint16_t)((CLOCK)/(BAUD_RATE * 16)) - 32; 			
	UART1->BDH &= ~UART_BDH_SBR_MASK;
	UART1->BDH |= UART_BDH_SBR(0x1F & (sbr>>8));
	UART1->BDL = UART_BDL_SBR(sbr);
	//UART1->C4 |= UARTLP_C4_OSR(OVERSAMPLE_RATE-1);
	
	UART1->BDH |= UART_BDH_LBKDIE(0);  
	UART1->BDH |= UART_BDH_RXEDGIE(0); 
	UART1->BDH |= UART_BDH_SBNS(0);
	
	
	UART1->C1 |= UART_C1_M(0); 												  
  UART1->C1 |= UART_C1_LOOPS(0); 
	UART1->C1	|= UART_C1_PE(0);
	
	UART1->C3 = UART_C3_ORIE(0)| UART_C3_NEIE(0) | UART_C3_FEIE(0) | UART_C3_PEIE(0);	
	
	UART1->S2 &= ~UARTLP_S2_MSBF_MASK; 
	UART1 -> S2 = UART_S2_RXINV(0); 
	UART1->C2 |= UART_C2_RE(1) | UART_C2_RIE(1);
...
void UART1_IRQHandler(void)
{
	...
	{
		received_go_signal = UART1 -> D; 
		if(received_go_signal)
			raise_this_flag = 1; 
		else
			raise_this_flag = 0; 
	}
...
void CombineReceiveBuffer(){
	...
	receivedcal_val = receivedbuffer[10]; 
}
