...
void Init_UART(void)
{
  ...
  // but UART0 clock needs to be set - to 48 MHz clock in this case
	...
	PORTA->PCR[RX_PIN] |= PORT_PCR_MUX(3); // Rx
  ...
  UART0->C1 |= UART0_C1_LOOPS(0); //0: Normal operation - RxD and TxD use separate pins.
	...
	// Clear error flags
	UART0->S1 |= UART0_S1_OR_MASK | UART0_S1_NF_MASK | UART0_S1_FE_MASK | UART0_S1_PF_MASK; //To clear these flags write logic one to them 
