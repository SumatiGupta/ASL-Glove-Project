...
void Init_UART(void)
{
  ...
  // but UART0 clock needs to be set - to 48 MHz clock in this case
	...
	//PORTE->PCR[TX_PIN] |= PORT_PCR_MUX(4); // Rx
  ...
  UART0->S2 = UART0_S2_MSBF(0) | UART0_S2_RXINV(0); //Send LSB first, do not invert received data
	...
	//UART0->C2 |= UART_C2_TE(1); // Enable UART receiver
...
void CombineReceiveBuffer(){
	...
	receivedpressure1 = COMBINE(receivedbuffer[11], receivedbuffer[10]);
	receivedpressure2 = COMBINE(receivedbuffer[13], receivedbuffer[12]);
	receivedcal_val = receivedbuffer[14]; 
}
